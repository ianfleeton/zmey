  ...
  describe ".initialize" do
    it "creates an image" do
      expect(Image).to receive(:new)
        .with(hash_including(file: upload, name: "image"))
        .and_return(double(Image).as_null_object)
      ImageUploader.new(file: upload, name: "image")
    end

    it "yields each image if block given" do
      image = nil
      ImageUploader.new(file: upload, name: "image") do |yielded|
        image = yielded
      end
    ...
    it "makes accessible the saved images via #images" do
      ...
      allow(Image).to receive(:new).and_return(image)
      uploader = ImageUploader.new(file: upload, name: "image")
    ...
    context "with two images in a ZIP archive" do
      ...
      it "creates both images" do
        expect { ImageUploader.new(file: upload, name: "image") }.to change { Image.count }.by(2)
      end
    end
  end
end
